...
def dibujar_escenario():
    ...
    for y in range(14):
        plot_sprite(0, y*4, ladrillos)
        plot_sprite(152, y*4, ladrillos)

    # ── Dibujo del suelo
    for x in range(18):
        plot_sprite(x*8 + 8, 8 * 5, suelo)

    # ── Dibujo de la estalactita
    plot_sprite(44, 0, estalactita)
    plot_sprite(64, 0, estalactita)

    # ── Dibujo de la llave
    plot_sprite(36, 0, llave)
    plot_sprite(64, 8, llave)
    plot_sprite(116, 0, llave)
    plot_sprite(96, 32, llave)
...
def dibujar_personaje(fotograma):
    ...
    plot_sprite(xp, yp, personaje[fotograma % 4])
...
for i in range(90):
    ...
    pantalla_refresh()
    stdscr.addstr(12, 5, "¡Manic Miner en modo Texto!")
    stdscr.refresh()
    time.sleep(WAIT)
